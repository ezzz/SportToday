...
3. Ouvrir `services\ingest\.env`. S'il n'existe pas encore, le script créera une copie de `.env.example` au premier lancement. Renseigner au minimum `API_FOOTBALL_KEY` avec la clé API-Football personnelle. Cette clé active aussi API-Volleyball par défaut. Le Tennis utilise XMLTV et le scoreboard public ESPN sans clé ; Golf et Diamond League utilisent également leurs connecteurs POC publics optionnels. Les clés restent sur le PC et ne sont jamais affichées par le site.
