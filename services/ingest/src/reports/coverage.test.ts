...
test("mesure les chaînes alimentées, vides et absentes", () => {
  const events: SportEvent[] = [event("match-1", "Paris Saint Germain", "Real Madrid"), event("match-2", "Marseille", "Monaco")];
  ...
  assert.equal(coverage.expectedEventCount, 2);
...
function event(id: string, home: string, away: string): SportEvent {
  return {
    id,
    source: "api-football",
    sourceEventId: id,
